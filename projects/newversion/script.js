...
let viz = d3.select("#vizContainer").append("svg")
    .style("width", wW)
    .style("height", wH) //wH*5
    .style("background-color", "rgba(255, 254, 250, 1)")
;
...
let graphTranslateScale3 = d3.scaleLinear().domain([0, 1]).range([-2*wH, -3*wH])
...
tooltip.append("rect").attr("x", 0).attr("y", 0).attr("width", 300).attr("height", 100).attr("fill", "rgba(255, 248, 225, 1)");
let description = tooltip.append("text").text("description").attr("x", 15).attr("y",20).attr("fill","black");
   ...
   d3.json("location.json").then(function (location){
       d3.json("PAC.json").then(function (pacData){
        ...
        let g2yAxisGroup = bothGraphs.append("g")
              ...
              .attr("transform", "translate("+paddingLeft*1.5+",0)")
        ;
        ...
        let projection = d3.geoNaturalEarth1()  //can change to other projections
          ...
          .scale(300)
          // .fitExtent([[-100,wH],[gW+100,gH+wH]],geoData)
     ...
     //.on("tick",simulationRan)
       ...
       function getPosition2(){
        ...
        .force("collide",d3.forceCollide(2.3))
      //.on("tick",simulationRan)
        .tick(400)
        .on("end",function(){
            pacData.forEach(d=>{
      ...
      function getPosition5(){
       ...
       .force("forceY",d3.forceY(function(d){
        ...
        if(setting =="1 =Public speech" || setting =="2 =Parliament" || setting =="3 =Public letter/ Statement" || setting =="4 =Resolution/law"){
          return wH*4 + paddingTop + 20;
        }
        else if (setting =="12 =Twitter" || setting == "50 =Other"){
          return wH*4 + paddingTop + 380;

        }
        else {
          return  wH*4 + paddingTop + 200;
        }
          ...
          // ;
            ...
            updatingData
              ...
              updatingData 
                ...
                tooltip.raise();
                tooltip.transition().duration(10).attr("transform", function(){
                  return "translate("+(d.g2x-150)+","+(d.g2y-120)+")"
                });
               // tooltip.selectAll("text").attr("fill","red");
                tooltip.transition().delay(20).attr("opacity", 2);
              })
      ...
      enterView({
          ...
          enterView({
            ...
            progress: function(el, progress) {
              ...
              console.log("progress", progress)
              bothGraphs.attr("transform", function(){
                let y = graphTranslateScale2(progress)
                return "translate(0, "+y+")";
              })
            },
            offset: 0.5, // enter at middle of viewport
          ...
          enterView({
            ...
            progress: function(el, progress) {
              ...
              console.log("progress", progress)
              bothGraphs.attr("transform", function(){
                let y = graphTranslateScale3(progress)
                return "translate(0, "+y+")";
              })
            },
            offset: 0.5, // enter at middle of viewport
          ...
          enterView({
            ...
            progress: function(el, progress) {
              ...
              console.log("progress", progress)
              bothGraphs.attr("transform", function(){
                let y = graphTranslateScale4(progress)
                return "translate(0, "+y+")";
              })
            },
            offset: 0.5, // enter at middle of viewport
